...
int fs_dirlist_push(FsDirlist *list, const struct dirent *de) {
        ...
        /* Calculate the record size properly for Zephyr compatibility */
#ifdef __ZEPHYR__
        sz = offsetof(struct dirent, d_name) + strlen(de->d_name) + 1;
#else
        sz = dirent_reclen(de) - offsetof(struct dirent, d_name);
#endif
