...
static void socket_buffer_get_line_cursor(SocketBuffer *buffer, char **datap, size_t **posp) {
        ...
        *datap = (char *)buffer->vecs[0].iov_base + buffer->vecs[0].iov_len;
...
static bool socket_buffer_consume(SocketBuffer *buffer, size_t n) {
        ...
        for ( ; !socket_buffer_is_consumed(buffer); ++buffer->writer) {
                ...
                if (t) {
                        ...
                        buffer->writer->iov_base = (char *)buffer->writer->iov_base + t;
                        n -= t;
                }
...
int socket_dequeue(Socket *socket, Message **messagep) {
        ...
        if (!socket->in.message) {
                ...
                r = iqueue_set_target(&socket->in.queue,
                                      (char *)message->data + sizeof(socket->in.header),
...
int socket_dispatch(Socket *socket, uint32_t event) {
        ...
        case EPOLLHUP:
                socket_hangup_output(socket);
                /*
                 * On Zephyr's AF_UNIX poll, POLLHUP may arrive without
                 * POLLIN even when the peer is fully disconnected (no
                 * pending data).  We must try to read here so that
                 * socket_dispatch_read() can detect and handle the EOF
                 * via socket_hangup_input().  Without this explicit
                 * read, the broker would never learn the peer went
                 * away, causing unique-name (:1.x) leaks.
                 *
                 * If the POLLHUP was premature (stale event, not yet
                 * disconnected), the read simply succeeds and processes
                 * any available data normally — no harm done.
                 */
                socket_dispatch_read(socket);
                break;
        }
